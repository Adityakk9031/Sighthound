...
mod file_pattern_tests {
    ...
    fn test_include_patterns_functionality() {
        ...
        assert!(!test_only_findings.is_empty(), "test_function should be found in test file with include pattern");
    ...
    fn test_combined_include_exclude_patterns() {
        ...
        assert_eq!(config_findings.len(), 2, "Should find config issues in 2 files (config and settings)");
    ...
    fn test_glob_pattern_matching() {
        ...
        // Load rules to test the pattern matching logic
        let rules = Rules::load_from_file(rules_file.path().to_str().unwrap())
            .expect("Failed to load rules");
        
        // We can't directly test the private matches_glob_pattern method,
        // but we can verify the rules loaded correctly with the expected patterns
        let malware_rules = rules.malware_detection.unwrap();
        assert_eq!(malware_rules.len(), 4);
        
        // Verify pattern configurations
        let test_rule = &malware_rules[0];
        ...
        let danger_rule = &malware_rules[1];
        ...
        let config_rule = &malware_rules[2];
